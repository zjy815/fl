...
if __name__ == '__main__':
    ...
    if(choice_1=='主页'):
        ...
        df = pd.read_csv('D:/study/py/Federated-Grade-Prediction-main/data/样本数据.csv')
        st.dataframe(df)
        ...
        net_glob.train()
        if args.load_fed != 'n':
            fed_model_path = 'D:/study/py/Federated-Grade-Prediction-main/save/' + args.load_fed + '.pt'
        ...
        for iter in range(epochs+1):
            ...
            if iter % args.save_every==args.save_every-1:
                model_save_path = 'D:/study/py/Federated-Grade-Prediction-main/save/accs_'+ args.alg + '_' + args.dataset + '_' + str(num_users) +'_'+ str(args.shard_per_user) +'_iter' + str(iter)+ '.pt'
        ...
        base_dir = 'D:/study/py/Federated-Grade-Prediction-main/save/accs_' + args.alg + '_' +  args.dataset + str(num_users) +'.csv'
        base_dir2 = 'D:/study/py/Federated-Grade-Prediction-main/save/losses_' + args.alg + '_' +  args.dataset + str(num_users) +'.csv'
        ...
        for i in range(len(options)):
            if (options[i] == '预测精度'):
                ...
                accs = pd.read_csv('D:/study/py/Federated-Grade-Prediction-main/save/accs_fedrep_mnist' + str(num_users) + '.csv')
                st.line_chart(accs)
            elif (options[i] == '损失函数'):
                st.subheader("函数损失趋势图")
                losses = pd.read_csv('D:/study/py/Federated-Grade-Prediction-main/save/losses_fedrep_mnist' + str(num_users) + '.csv')
                st.line_chart(losses)
            elif (options[i] == '混淆矩阵'):
                st.subheader("混淆矩阵图")
                image = Image.open('D:/study/py/Federated-Grade-Prediction-main/save/confusion_matrix.png')
